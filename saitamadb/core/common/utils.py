...
from saitamadb.core.common import params as db_params
...
def validate_update_data(data: Dict[str, object], schema: SchemaDictType) -> bool:
    if not all(i in schema for i in data.keys()):
        raise c_err.DataError("Unknown key found in data")
...
def generate_hash_id(values: List[str]) -> str:
    """Generate SHA256 check sum by combining all the entries inside the values list,
