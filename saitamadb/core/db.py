...
from saitamadb.core.common import utils as db_utils
from saitamadb.errors import schema_error as s_err
from saitamadb.errors import common_errors as c_err
...
class SaitamaDb:
    ...
    def _load_initial_schema(self) -> None:
        ...
        if schema:
            ...
            if not self._schema:
                raise s_err.SchemaError("The schema is not provided")
            else:
                if not self._in_memory:
    ...
    def add(self, values: List[Dict[str, object]], get_hash_id: bool = False) -> Union[None, List[str]]:
        ...
        for data in values:
            if self._schema:
                if db_utils.validate_data_with_schema(data, self._schema):
                    data = db_utils.add_default_to_data(data, self._schema)
        ...
        try:
            ...
            raise c_err.DataDuplicateError(
    ...
    def get_by_query(self, query: Dict[str, object]) -> GetType:
        ...
        if self._schema:
            if db_utils.validate_query_data(query, self._schema):
                key = list(query)[0]
    ...
    def get_hash_id(self, condition: Dict[str, object]) -> List[str]:
        ...
        if self._schema:
            if db_utils.validate_update_data(condition, self._schema):
    ...
    def update_by_query(self, query: Dict[str, object], update_data: DBDataType) -> Dict[str, str]:
        ...
        if self._schema:
            if db_utils.validate_query_data(query, self._schema) and db_utils.validate_update_data(update_data,
                                                                                                   self._schema):
                q_key = list(query)[0]
                q_val = query[q_key]
    ...
    def update_by_hash_id(self, hash_id: str, update_data: DBDataType) -> Dict[str, str]:
        ...
        if hash_id in u_db.index:
            if self._schema:
                if db_utils.validate_update_data(update_data, self._schema):
                    for key, val in update_data.items():
    ...
    def delete_by_hash_id(self, hash_id: str) -> None:
        """Delete a records from thr DB based on their hash_id"""
    ...
    def raw_db(self) -> pd.DataFrame:
        """Returns the in memory representation of the DB"""
    ...
    def commit(self) -> None:
        ...
        if isinstance(self._db, pd.DataFrame):
            if not self._in_memory:
                db_utils.dump_db(self._db, self._db_path, self._db_name)
    ...
    def _get_hash(self, values: List[str], hash_list: List[str]) -> str:
        ...
        def gen_dupe_hash(extra: int = 0) -> str:
            if extra:
                hash_ = db_utils.generate_hash_id(values + [str(extra)])
            else:
                hash_ = db_utils.generate_hash_id(values)
        ...
        if not self._data_dupe:
            return db_utils.generate_hash_id(values)
